...
{	
	...
	private int test_removeCat_0(int testIdx) 
	{
		...
		try {
			...
			CatInfo b = new CatInfo("B", 85, 60, 240, 30);
			CatInfo c = new CatInfo("C", 88, 70, 248, 10);
			CatInfo d = new CatInfo("D", 86, 55, 243, 40);
			
			CatTree t = new CatTree(a);
			t.addCat(b);
			//t.addCat(d);
			System.out.println("ORIGINAL "+ t.root);
			...
			System.out.println("AFTER REMOVAL " + t.root);
			
		
			
			
			if (t.root.data.equals(b) && t.root.same == null && t.root.senior == null && t.root.junior.data.equals(d)) {
				grade += 3;
			} else{
	...
	private int test_mostSenior_1(int testIdx) 
	{
		...
		try {
			...
			CatInfo c = new CatInfo("C", 88, 70, 248, 10);
			CatInfo d = new CatInfo("D", 95, 55, 245, 50);
			CatInfo e = new CatInfo("E", 86, 55, 245, 20);
			...
			if (most == 86) {
				...
				comment = comment + "Error: MostSenior is incorrect.\n";
			}
	...
	{
		String comment = "[" + testIdx + "]: Test fluffiest when root isn't fluffiest.\n";
		int maxScore = 3;
		int grade = 0;
		...
		try {
			...
			if (most == 70) {
				...
				comment = comment + "Error: fluffiest is incorrect when the root isn't the fluffiest.";
			}
	...
	private int test_fluffyFrom_1(int testIdx) 
	{
		String comment = "[" + testIdx + "]: Test fluffiestFromMonth when Cat is in the 'same' branch.\n";
		int maxScore = 2;
		int grade = 0;
		...
		try {
			...
			CatInfo ret = t.root.fluffiestFromMonth(60);
			if (ret.equals(b)) {
				grade += 2;
			} else {
				comment = comment + "Error: fluffiestFromMonth when Cat is in the 'different' branch.\n";
			}
	...
	private int test_costPlan_1(int nbMonths) 
	{
		String comment = "[" + nbMonths + "]: Test costPlanning - test 1.\n";
		int maxScore = 2;
		int grade = 0;

		try {
			CatInfo a = new CatInfo("A", 100, 40, 243, 20);
			CatInfo b=  new CatInfo("B", 100, 60, 245, 20);
			CatInfo c = new CatInfo("C", 125, 75, 248, 20);
			CatInfo d = new CatInfo("D", 100, 15, 245, 20);
			...
			int []plan = t.root.costPlanning(6);
			if (plan[2] == 20) {
				grade += 2;
			} 
			
			else {
				System.out.println(plan[0]+plan[1]+plan[2]+plan[3]+plan[4]+plan[5]);
	...
	public static void main(String[] args) 
	{
		...
		total+=m.test_costPlan_1(14);
		total+=m.test_mostSenior_1(15);
		total+=m.test_fluffiest_1(16);
